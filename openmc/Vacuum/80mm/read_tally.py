...
'''
tally2 = sp.get_tally(name='mesh_flux')
flux = tally2.get_slice(scores=['flux'])
flux.std_dev.shape = (100, 100)
flux.mean.shape = (100, 100)
fig = plt.subplot(121)
fig.imshow(flux.mean, vmin=0, vmax=0.001)
plt.show()
'''
...
for i in range(0, len(filtered_time)-50, len(filtered_time)-50):
    for j in range(i+50, len(filtered_time), 10):
        time_interval = filtered_time[0:j]
        flux_interval = filtered_flux[0:j]
...
plt.scatter(data['Start time [ms]'], data['End time [ms]'], c=data['Alpha'], vmin=5, vmax=10) #, vmax=15) #, norm=colors.LogNorm())
# plt.xlim(0, 1.05E-3)
