...
filtered_data = df[(df['time low [s]'] >= 6.6E-4) & (df['time low [s]'] <= 1.4E-3)]
...
plt.scatter(data['Start time [ms]'], data['End time [ms]'], c=data['Alpha'], vmin=5, vmax=15) #, vmax=15) #, norm=colors.LogNorm())
# plt.xlim(0, 1.05E-3)
...
plt.scatter(df['time low [s]'], df['mean'], s=0.1)
#plt.plot(filtered_time*1000, filtered_flux)
plt.xlim(0.00066, 0.0009)
plt.ylim(0, 3e-6)
plt.xlabel('Time [ms]')
plt.ylabel('Neutron current [particle/source]')
