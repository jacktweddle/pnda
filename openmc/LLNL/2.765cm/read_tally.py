...
filtered_data = df[(df['time low [s]'] >= 6.6E-4) & (df['time low [s]'] <= 3E-3)]
...
for i in range(0, len(filtered_time)-50, 200):
    for j in range(i+50, len(filtered_time), 50):
        time_interval = filtered_time[20:j]
        flux_interval = filtered_flux[20:j]
        ...
        if np.sqrt(pcov[1, 1]) < low_unc and popt[1] > 18:
        ...
        if np.sqrt(pcov[1, 1])/popt[1] < low_rel_err and popt[1] > 18:
...
plt.scatter(data['Start time [ms]'], data['End time [ms]'], c=data['Alpha'], vmin=20, vmax=35) #, vmax=15) #, norm=colors.LogNorm())
# plt.xlim(0, 1.05E-3)
...
t_vals = np.linspace(0.66, 1.2, 1000)
