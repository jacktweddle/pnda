...
for i in range(0, len(filtered_time)-50, 50):
    for j in range(i+50, len(filtered_time), 50):
        ...
        if np.sqrt(pcov[1, 1]) < low_unc and popt[1] > 15:
        ...
        if np.sqrt(pcov[1, 1])/popt[1] < low_rel_err and popt[1] > 15:
...
plt.scatter(data['Start time [ms]'], data['End time [ms]'], c=data['Alpha'], vmin=10, vmax=30) #, vmax=15) #, norm=colors.LogNorm())
# plt.xlim(0, 1.05E-3)
