...
filtered_data = df[(df['time low [s]'] >= 6.6E-4) & (df['time low [s]'] <= 9.2E-4)]
...
for i in range(0, len(filtered_time)-50, 10):
    for j in range(i+50, len(filtered_time), 10):
        ...
        if np.sqrt(pcov[1, 1]) < low_unc and popt[1] > 10:
        ...
        if np.sqrt(pcov[1, 1])/popt[1] < low_rel_err and popt[1] > 10:
